<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="名称" align="center" prop="planName" width="200" />
...
const handleRowAdd = () => {
  nursingPalnList.value.push(createDefaultPlanItem());
};
//删除行数据
...
function createDefaultPlanItem() {
  return {
    projectId: '',
    executeTime: '',
    executeCycle: '',
...
function reset() {
  ...
  nursingPalnList.value = [createDefaultPlanItem()];
...
const getDetails = async (id) => {
  try {
    ...
    const response = await getNursingPlan(id);
    formData.value = response.data;
    formData.value.status = String(formData.value.status);
    nursingPalnList.value = formData.value.projectPlans?.length
      ? formData.value.projectPlans.map((item) => ({
          ...item,
          executeCycle: item.executeCycle !== undefined && item.executeCycle !== null ? String(item.executeCycle) : '',
        }))
...
function validateProjectPlans() {
  if (!nursingPalnList.value.length) {
    proxy.$modal.msgError('请至少添加一条护理项目');
    return false;
  }

  for (let i = 0; i < nursingPalnList.value.length; i++) {
    const item = nursingPalnList.value[i];
    if (!item.projectId || !item.executeTime || !item.executeCycle || !item.executeFrequency) {
      proxy.$modal.msgError(`请完善第${i + 1}条护理项目信息`);
      return false;
    }
  }
...
function hasDuplicateIds(objectsArray) {
  ...
  for (const obj of objectsArray) {
    // 假设每个对象都有一个id属性
    if (obj.projectId !== undefined && obj.projectId !== null && obj.projectId !== '') {
      // 尝试将id添加到Set中
