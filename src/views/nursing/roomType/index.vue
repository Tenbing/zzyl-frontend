...
function getList() {
  loading.value = true;
  listRoomType(queryParams.value)
    .then(response => {
      const rows = response?.rows ?? response?.data?.rows ?? response?.data?.records ?? response?.data ?? [];
      const totalCount = response?.total ?? response?.data?.total ?? rows.length;
      roomTypeList.value = Array.isArray(rows) ? rows : [];
